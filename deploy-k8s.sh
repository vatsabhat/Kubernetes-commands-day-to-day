#!/bin/bash
# Comprehensive HA Kubernetes Deployment Script for Ubuntu
set -e
...
sudo sed -i 's/SystemdCgroup = false/SystemdCgroup = true/g' /etc/etc/containerd/config.toml || sudo sed -i 's/SystemdCgroup = false/SystemdCgroup = true/g' /etc/containerd/config.toml
...
EOF
    ...
    # Configure kubectl for root/current user
    mkdir -p $HOME/.kube
    ...
    echo "=========================================================="
    echo "First node complete. Run the outputted 'kubeadm join' command"
    echo "with the '--control-plane --certificate-key' flags on Node 2."
    echo "=========================================================="
else
    echo "=========================================================="
    echo "Prerequisites and VIP engine installed on Node 2."
    echo "Please execute the 'kubeadm join' token string generated by Node 1"
    echo "to securely bind this secondary control plane."
    echo "=========================================================="
fi
